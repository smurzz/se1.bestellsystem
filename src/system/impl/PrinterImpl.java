...
class PrinterImpl implements Printer{
	...
	public StringBuffer printOrder(Order order) {
		...
		for (int i = 0; i < items.length; i++) {
			StringBuffer pricePiece = formatter.fmtPrice(items[i].getArticle().getUnitPrice(), items[i].getArticle().getCurrency());
			totalPrice = items[i].getArticle().getUnitPrice() * items[i].getUnitsOrdered();
			double taxPrice = calc.calculateIncludedVAT(items[i].getArticle().getUnitPrice(), items[i].getArticle().getTax()) / 100.0;
