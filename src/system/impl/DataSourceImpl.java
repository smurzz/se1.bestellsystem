...
class DataSourceImpl implements DataSource {
	...
	public long importCustomerJSON(String jsonFileName, CustomerRepository collector, Integer... limit) {
		long count = read(jsonFileName, jsonNode -> createCustomer(jsonNode), e -> collector.save(e), limit);
		return count;
	}

	@Override
	public long importArticleJSON(String jsonFileName, ArticleRepository collector, Integer... limit) {
		long count = read(jsonFileName, jsonNode -> createArticle(jsonNode), e -> collector.save(e), limit);
		return count;
	}
	...
	private <T> long read(String jsonFileName, Function<JsonNode, Optional<T>> creator, Consumer<T> collector,
			Integer... limit) {
		int lim = Math.max(limit.length > 0 ? limit[0].intValue() : Integer.MAX_VALUE, 0);
		long count = 0;
		try (
				// auto-close on exception, InputStream implements the java.lang.AutoClosable
				// interface
	...
	/**
	 * Factory method that creates a new Customer object with attribute values from
	 * a JSON Node. Methods returns Optional<Customer>, which can be empty if no
	 * valid object could be created from JSON
	 * 
	 * @param jsonNode JSON Node with Customer attributes
	 * @return Optional<Customer> with object with valid attributes from JSON Node
	 *         or empty
	 */
	 ...
	 * @param mandatory     if attributeName could not be found, throws
	 *                      InvalidParameterException for mandatory attributes,
	 *                      method returns "" otherwise
	 * @return String value for attributeName if found, "" if not for non-mandatory
	 *         attributes
	 * @throws InvalidParameterException if attributeName cannot be found and
	 *                                   mandatory is true
	 */

	private String valueAsString(JsonNode jsonNode, String attributeName, boolean mandatory)
...
//		valid = valid && name != null && name.length() > 0;
	 ...
	 * @return Optional<Article> with object with valid attributes from JSON Node or
	 *         empty
	 */
	...
	private Optional<Article> createArticle(JsonNode jsonNode) {
		...
		try {
			...
			//
			Currency currency = currencyStr.equalsIgnoreCase("EUR") || currencyStr.equalsIgnoreCase("EURO")
					? Currency.EUR
			...
			if (validArticleAttrs(id, price, description, currency)) {
				Article article = new Article() // create new Article object
						.setId(id) // and initialize attributes
						.setCurrency(currency).setDescription(description).setUnitPrice(price);
				//
